...
The most opinionated generator. It runs, in order: model, create-table migration, controller (pluralised from `<Name>`), views (`index`, `show`, `new`, `edit`, `_form`), a model spec and a full CRUD controller spec under `tests/specs/`, and a `.resources()` route in `config/routes.cfm`. If any step fails the whole batch is rolled back so you're not left with a half-generated resource.
