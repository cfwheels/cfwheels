...
test_url="http://127.0.0.1:${port}/wheels/testbox?db=${db}&format=json&only=failure,error&method=runRemote&directory=&testSpecs=cfmlerror%20shows%20wheels%20templates&testBundles=wheels%2Etests_testbox%2Especs%2Eevents%2Eonerror&opt_run=true&coverageEnabled=false"
