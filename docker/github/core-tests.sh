...
test_url="http://127.0.0.1:${port}/wheels/testbox?db=${db}&only=failure,error&method=runRemote&directory=&testSpecs=findall%20respects%20model%20config%20datasource&testBundles=wheels%2Etests_testbox%2Especs%2Emodel%2Eread&opt_run=true&coverageEnabled=false"
