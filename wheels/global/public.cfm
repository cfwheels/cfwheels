...
<cffunction name="URLFor" returntype="string" access="public" output="false" hint="Creates an internal URL based on supplied arguments.">
	...
	<cfscript>
		...
		{
			if (not(Len(arguments.route)) and not(Len(arguments.controller)) and not(Len(arguments.action)))
			...
			if (Len(arguments.route) and (Len(arguments.controller) or Len(arguments.action) or (IsObject(arguments.key) or Len(arguments.key))))
			...
			if (arguments.onlyPath and (Len(arguments.host) or Len(arguments.protocol)))
		...
		if (application.wheels.URLRewriting neq "Off")
		{
		...
		if (not(arguments.onlyPath))
		{
			...
			else if (cgi.server_port neq 80)
...
<cffunction name="obfuscateParam" returntype="string" access="public" output="false" hint="Obfuscates a value.">
	...
	<cfscript>
		var loc = StructNew();
		if (IsValid("integer", arguments.param) and IsNumeric(arguments.param) and arguments.param gt 0)
		{
...
<cffunction name="deobfuscateParam" returntype="string" access="public" output="false" hint="Deobfuscates a value.">
	...
	<cfscript>
		var loc = StructNew();
		if (Val(arguments.param) neq arguments.param)
		{
			try
			{
				...
				if (Left(ToString(FormatBaseN((loc.checksumtest+154),10)),2) neq Left(InputBasen(loc.checksum, 16),2))
					loc.returnValue = arguments.param;
			}
			catch(Any e)
			{
...
<cffunction name="addRoute" returntype="void" access="public" output="false" hint="Adds a new route to your application.">
	...
	<cfscript>
		...
		if (not(Len(arguments.controller)) and arguments.pattern Does Not Contain "[controller]")
		...
		if (not(Len(arguments.action)) and arguments.pattern Does Not Contain "[action]")
