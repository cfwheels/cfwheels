...
<cffunction name="$createControllerClass" returntype="any" access="public" output="false">
	...
	<cfscript>
		var loc = {};
		application.wheels.controllers[arguments.name] = $createObjectFromRoot(path=application.wheels.controllerComponentPath, fileName=$controllerFileName(arguments.name), method="$initControllerClass", name=arguments.name);
