...
<p class="intro">A quick tutorial that demonstrates how quickly you can get database connectivity up and running with Wheels.</p>
...
Wheels supports MySQL, SQL Server, PostgreSQL, Oracle, and H2. It doesn't matter which DBMS you use. We will all be writing the same CFML code to interact with the database. Wheels does everything behind the scenes that needs to be done to work with each DBMS.
...
To generate the form tag's `action` attribute, the `startFormTag()` function takes parameters similar to the `linkTo()` function that we introduced in the [Hello World][1] tutorial. We can pass in `controller`, `action`, `key`, and other route- and parameter-defined URLs just like we do with `linkTo()`.
...
After the update, we'll add a success message [Using the Flash][2] and send the end user back to the edit form in case they want to make more changes.
