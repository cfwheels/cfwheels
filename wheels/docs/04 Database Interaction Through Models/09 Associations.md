...
<p class="introduction">Through some simple configuration, Wheels allows you to unlock some powerful functionality to use your database tables' relationships in your code.</p>
...
Let's say that you tell Wheels through a `hasMany()` call that a `post` _has many_ `comments`. What happens then is that Wheels will enrich the `post` model by adding a bunch of useful methods related to this association.
...
Given that you have told Wheels that a `post` _has many_ `comments` through a `hasMany()` call, here are the methods that will be made available to you on the `post` model.
