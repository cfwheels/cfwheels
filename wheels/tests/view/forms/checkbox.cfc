<cfcomponent extends="wheelsMapping.test">
	...
	<cffunction name="test_checked_when_property_value_equals_checkedValue">
		...
		<cfset loc.e = '<input checked="checked" id="user-birthdaymonth-11" name="user[birthdaymonth]" type="checkbox" value="11" /><input name="user[birthdaymonth]($checkbox)" type="hidden" value="0" />'>
		<cfset loc.r = loc.controller.checkBox(argumentcollection=loc.args)>
		<cfset assert("loc.e eq loc.r")>
		<cfset loc.args.checkedvalue = "12">
		<cfset halt(false, "loc.controller.checkBox(argumentcollection=loc.args)")>
		<cfset loc.e = '<input id="user-birthdaymonth-12" name="user[birthdaymonth]" type="checkbox" value="12" /><input name="user[birthdaymonth]($checkbox)" type="hidden" value="0" />'>
		<cfset loc.r = loc.controller.checkBox(argumentcollection=loc.args)>
		<cfset assert("loc.e eq loc.r")>
	</cffunction>
