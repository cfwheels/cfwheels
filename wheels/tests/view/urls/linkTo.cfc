<cfcomponent extends="wheelsMapping.test">
	...
	<cffunction name="test_controller_action_only">
		...
		<cfset loc.r = '<a href="#application.wheels.webpath#index.cfm/account/logout">Log Out</a>'>
		<cfset assert('loc.e eq loc.r')>
	</cffunction>
	
	<cffunction name="test_confirm_is_escaped">
		<cfset loc.e = loc.controller.linkTo(confirm="Mark as: 'Completed'?")>
		<cfset loc.r = '<a href="#application.wheels.webpath#index.cfm/" onclick="return confirm(''Mark as: \''Completed\''?'');">#application.wheels.webpath#index.cfm/</a>'>
		<cfset assert('loc.e eq loc.r')>
	</cffunction>	
