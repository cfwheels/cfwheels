component extends="wheels.tests.Test" {
	...
	function test_buttonto_inner_icon() {
		...
		expected = '<form action="#application.wheels.webpath#" class="form-class" confirm="confirm-value" disable="disable-value" method="post"><button class="input&##x20;class" type="submit" value="save">' & '<i class=''fa fa-icon'' /> Edit' & '</button>' & _controller.authenticityTokenField() & '</form>';
		assert('actual eq expected');
	}
