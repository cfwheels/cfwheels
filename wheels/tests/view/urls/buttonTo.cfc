component extends="wheels.tests.Test" {
	...
	function test_buttonto_with_delete_method_argument() {
		...
		expected = '<form action="#application.wheels.webpath#" method="post"><input id="_method" name="_method" type="hidden" value="delete"><input type="submit" value=""></form>';
		assert('actual eq expected');
	}

	function test_buttonto_with_put_method_argument() {
		actual = _controller.buttonTo(method="put");
		expected = '<form action="#application.wheels.webpath#" method="post"><input id="_method" name="_method" type="hidden" value="put"><input type="submit" value=""></form>';
		assert('actual eq expected');
	}
