<cfcomponent extends="wheelsMapping.Test">
	...
	<cffunction name="test_set_select_once">
		...
		<cfset assert('loc.profileModel.query().select eq $listClean(loc.select)')>
	</cffunction>
	...
	<cffunction name="test_set_select_multiple_times">
		...
		<cfset assert('loc.profileModel.query().select eq $listClean(ListAppend(loc.select1, loc.select2))')>
	</cffunction>
