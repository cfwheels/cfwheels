<cfcomponent extends="wheelsMapping.test">
	...
	<cfset load_Models("users,photogalleryphotos,photogalleries")>
	...
	<cffunction name="test_5_records_2_perpage_3_pages">
		<cfset loc.r = global.user.findAll(select="id", order="id")>
		
		<!--- 1st page --->
		<cfset loc.e = global.user.findAll(select="id", perpage="2", page="1", handle="pagination_test_2", order="id")>
	...
	<cffunction name="test_specify_where_on_joined_table">
		...
		<cfset loc.r = global.photogalleryphoto.findAll(argumentCollection=loc.args2)>
		
		<!--- page 1 --->	
		<cfset loc.e = global.photogalleryphoto.findAll(argumentCollection=loc.args)>
