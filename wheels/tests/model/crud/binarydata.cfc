<cfcomponent extends="wheelsMapping.Test">
	...
	<cffunction name="setup">
 	...
 	<cffunction name="test_insert">
		<cfset loc.gallery = model("gallery").findOne(include="user", where="users.lastname = 'Petruzzi'", orderby="id")>
		<cfif NOT StructKeyExists(server, "bluedragon")>
			<cftransaction action="begin">
				<cfset loc.photo = model("photo").create(galleryid="#loc.gallery.id#", filename="somefilename", fileData=loc.binaryData, description1="something something")>
				<cfset loc.photo = model("photo").findByKey(loc.photo.id)>
				<cfset loc._binary = loc.photo.filedata>
				<cftransaction action="rollback">
			</cftransaction>
