<cfcomponent extends="wheelsMapping.test">
	...
	<cffunction name="test_select_distinct_addresses">
		<cfset loc.q = model("user").findAll(select="address", distinct="true", order="address")>
...
<!---
	...
	<cffunction name="test_select_users_groupby_address">
		<cfset loc.q = model("user").findAll(select="address", group="address", order="address", result="loc.result")>
		<cfset assert('loc.q.recordcount eq 4')>
		<cfset loc.e = "123 Petruzzi St.|456 Peters Dr.|789 Djurner Ave.|987 Riera Blvd.">
		<cfset loc.r = valuelist(loc.q.address, "|")>
		<cfset assert('loc.e eq loc.r')>
 ...
 --->
	...
	<cffunction name="test_findByKey">
		<cfset loc.e = model("user").findOne(where="lastname = 'petruzzi'")>
		<cfset loc.q = model("user").findByKey(loc.e.id)>
	...
	<cffunction name="test_findOne">
 	...
 	<cffunction name="test_findAll">
		<cfset loc.q = model("user").findAll()>
		<cfset assert('loc.q.recordcount eq 5')>
		<cfset loc.q = model("user").findAll(where="lastname = 'petruzzi' OR lastname = 'peters'", order="lastname")>
	...
	<cffunction name="test_findOneByXXX">
		<cfset loc.q = model("user").findOneByFirstname('per')>
		<cfset assert('isobject(loc.q)')>
		<cfset loc.q = model("user").findOneByZipcode(value="22222", order="id")>
		<cfset assert('isobject(loc.q)')>
	</cffunction>

	<cffunction name="test_findAllByXXX">
		<cfset loc.q = model("user").findAllByZipcode(value="22222", order="id")>
		<cfset assert('loc.q.recordcount eq 2')>
		<cfset loc.q = model("user").findAllByZipcode(value="11111", order="id")>
		<cfset assert('loc.q.recordcount eq 1')>
	</cffunction>

	<cffunction name="test_findByKey_norecords_returns_correct_type">
		<cfset loc.q = model("user").findByKey("0")>
		<cfset halt(false, 'loc.q')>
		<cfset assert('isboolean(loc.q) and loc.q eq false')>

		<cfset loc.q = model("user").findByKey(key="0", returnas="query")>
		<cfset halt(false, 'loc.q')>
		<cfset assert('isquery(loc.q) and loc.q.recordcount eq 0')>

		<cfset loc.q = model("user").findByKey(key="0", returnas="object")>
		<cfset halt(false, 'loc.q')>
		<cfset assert('isboolean(loc.q) and loc.q eq false')>

		<!--- readd when we have implemented the code to throw an error when an incorrect returnAs value is passed in
		<cfset loc.q = raised('model("user").findByKey(key="0", returnas="objects")')>
	...
	<cffunction name="test_findOne_norecords_returns_correct_type">
		<cfset loc.q = model("user").findOne(where="id = 0")>
		<cfset halt(false, 'loc.q')>
		<cfset assert('isboolean(loc.q) and loc.q eq false')>

		<cfset loc.q = model("user").findOne(where="id = 0", returnas="query")>
		<cfset halt(false, 'loc.q')>
		<cfset assert('isquery(loc.q) and loc.q.recordcount eq 0')>

		<cfset loc.q = model("user").findOne(where="id = 0", returnas="object")>
		<cfset halt(false, 'loc.q')>
		<cfset assert('isboolean(loc.q) and loc.q eq false')>

		<!--- readd when we have implemented the code to throw an error when an incorrect returnAs value is passed in
		<cfset loc.q = raised('model("user").findOne(where="id = 0", returnas="objects")')>
		<cfset loc.r = "Wheels.IncorrectArgumentValue">
		<cfset halt(false, 'loc.q')>
		<cfset assert('loc.q eq loc.r')> --->
	</cffunction>
	...
	<cffunction name="test_findAll_norecords_returns_correct_type">
		<cfset loc.q = model("user").findAll(where="id = 0")>
		<cfset halt(false, 'loc.q')>
		<cfset assert('isquery(loc.q) and loc.q.recordcount eq 0')>

		<cfset loc.q = model("user").findAll(where="id = 0", returnas="query")>
		<cfset halt(false, 'loc.q')>
		<cfset assert('isquery(loc.q) and loc.q.recordcount eq 0')>

		<!--- readd when we have implemented the code to throw an error when an incorrect returnAs value is passed in
		<cfset loc.q = raised('model("user").findAll(where="id = 0", returnas="object")')>
		<cfset loc.r = "Wheels.IncorrectArgumentValue">
		<cfset halt(false, 'loc.q')>
		<cfset assert('loc.q eq loc.r')> --->

		<cfset loc.q = model("user").findAll(where="id = 0", returnas="objects")>
		<cfset halt(false, 'loc.q')>
