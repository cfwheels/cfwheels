<cfcomponent extends="wheelsMapping.test">
 	...
 	<cffunction name="test_soft_delete">
		<cftransaction action="begin">
			...
			<cfset loc.postsWithoutSoftDeletes = model("Post").findAll(reload=true)>
			<cfset loc.postsWithSoftDeletes = model("Post").findAll(includeSoftDeletes=true, reload=true)>
			<cftransaction action="rollback" />
		</cftransaction>
 	...
 	<cffunction name="test_permanent_delete_of_soft_deleted_records">
		<cftransaction action="begin">
			...
			<cfset loc.post.delete()>
			<cfset loc.softDeletedPost = model("Post").findByKey(key=loc.post.id, includeSoftDeletes=true, reload=true)>
 	...
 	<cffunction name="test_delete_by_key">
		<cftransaction action="begin">
			<cfset loc.post = model("Author").findOne(reload=true)>
			<cfset model("Author").deleteByKey(loc.post.id)>
			<cfset loc.allAuthors = model("Author").findAll(reload=true)>
			<cftransaction action="rollback" />
		</cftransaction>
		<cfset assert('loc.allAuthors.recordcount eq 6')>
	</cffunction>
 	...
 	<cffunction name="test_soft_delete_by_key">
		<cftransaction action="begin">
			...
			<cfset loc.postsWithoutSoftDeletes = model("Post").findAll(includeSoftDeletes=false, reload=true)>
			<cfset loc.postsWithSoftDeletes = model("Post").findAll(includeSoftDeletes=true, reload=true)>
			<cftransaction action="rollback" />
		</cftransaction>
		<cfset assert('loc.postsWithoutSoftDeletes.recordcount eq 3 AND loc.postsWithSoftDeletes.recordcount eq 4')>
