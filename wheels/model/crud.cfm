...
<cffunction name="findAll" returntype="any" access="public" output="false"
	...
	<cfscript>
		...
		else if (!StructKeyExists(loc, "returnValue"))
		{
			...
			request.wheels[Hash(SerializeJSON(loc.findAll.query))] = variables.wheels.class.name; // place an identifer in request scope so we can reference this query when passed in to view functions
			if (arguments.returnAs == "query")
			{
