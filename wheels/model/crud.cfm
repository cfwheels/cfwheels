...
<cffunction name="findAll" returntype="any" access="public" output="false"
	...
	<cfscript>
		...
		else if (!StructKeyExists(loc, "returnValue"))
		{
			...
			loc.queryShellKey = $hashedKey(variables.wheels.class.name, arguments);
			...
			loc.queryKey = $hashedKey(variables.wheels.class.name, arguments, loc.originalWhere);
			...
			request.wheels[$hashedKey(loc.findAll.query)] = variables.wheels.class.name; // place an identifer in request scope so we can reference this query when passed in to view functions
			if (arguments.returnAs == "query")
			{
...
<cffunction name="save" returntype="boolean" access="public" output="false"
	...
	<cfscript>
		...
		if ($callback("beforeValidation"))
		{
			if (isNew())
			{
				if ($callback("beforeValidationOnCreate") && $validate("onSave,onCreate") && $callback("afterValidation") && $callback("afterValidationOnCreate") && $callback("beforeSave") && $callback("beforeCreate"))
				{
			...
			{
				if ($callback("beforeValidationOnUpdate") && $validate("onSave,onUpdate") && $callback("afterValidation") && $callback("afterValidationOnUpdate") && $callback("beforeSave") && $callback("beforeUpdate"))
				{
					if (hasChanged())
					{
