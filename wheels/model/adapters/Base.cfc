<cfcomponent output="false">
    ...
    */
	...
	<cffunction name="$performQuery" returntype="struct" access="public" output="false">
		...
		<cfargument name="name" type="string" required="false" default="query">
		...
		<cfscript>
			...
			local.args.name = "local." & arguments.name;
			...
			StructDelete(local.orgArgs, "name");
		...
		<cfscript>
			if (StructKeyExists(local, arguments.name)) {
				local.rv.query = local[arguments.name];
			}
