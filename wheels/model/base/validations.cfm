...
<cffunction name="valid" returntype="boolean" access="public" output="false" hint="Runs the validation on the object and returns `true` if it passes it. Wheels will run the validation process automatically whenever an object is saved to the database, but sometimes it's useful to be able to run this method to see if the object is valid without saving it to the database."
	...
	<cfscript>
		...
		{
			if ($validate("onSave") && $callback("afterValidation", arguments.callbacks))
...
<cffunction name="$initalizeValidations" returntype="void" access="public" output="false">
	<cfargument name="validations" type="string" required="true">
	<cfscript>
	var loc = {};
	arguments.validations = $listClean(arguments.validations);
	loc.iEnd = ListLen(arguments.validations);
	for (loc.i=1; loc.i <= loc.iEnd; loc.i++)
	{
		variables.wheels.class.validations[ListGetAt(arguments.validations, loc.i)] = ArrayNew(1);
	}
	</cfscript>
</cffunction>