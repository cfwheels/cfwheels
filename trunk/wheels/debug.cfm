...
<cfif application.settings.showDebugInformation AND isDefined("request.wheels.showDebugInformation") AND request.wheels.showDebugInformation>
	...
	<tr>
		...
		<td><cfif StructCount(application.wheels.plugins) IS NOT 0><cfloop collection="#application.wheels.plugins#" item="loc.i"><a href="<cfif application.wheels.URLRewriting IS 'On'>/plugins/#LCase(loc.i)#<cfelseif application.wheels.URLRewriting IS 'Partial'>/index.cfm/plugins/#LCase(loc.i)#<cfelse>/index.cfm?controller=plugins&action=#LCase(loc.i)#</cfif>">#loc.i#</a></cfloop><cfelse>None</cfif></td>
	</tr>
	<tr>
